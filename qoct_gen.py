...
class QH:
	...
	def __init__(self, H0, Hctrl, ctrl_i, phi_i, dt=.01):
		...
		self.H0 = np.array(H0)          #Hamiltonian with no control/laser
		self.Hctrl = np.array(Hctrl)    #Hamiltonian of control/laser term
		self.ctrl = np.array(ctrl_i)  #initial control/laser
	...
	def run(self):
		...
		for it in range(iter_time):
			...
			print 'IterTime: %s,   Error: %s,   TotTime: %s,   AvgTime: %s'\
				 %( it+1, 1-abs(fi[0,0]), clock()-start, (clock()-start)/(it+1))
